...
description: "Turning a long link into \"sho.rt/x7Qa\" sounds trivial — until you need it to handle a billion links and never hand out the same code twice."
extractedReferences:
...
Someone pastes a 400-character tracking URL into a tweet and it eats half their character budget. You want to hand them back `sho.rt/x7Qa` instead, and when anyone clicks it, send them straight to the original. Easy — a table with two columns.

Then marketing runs a campaign, the short link goes out to ten million inboxes, and every click hits your one database row at the same instant. The "trivial" version melts. The real problem isn't shortening a link once; it's serving billions of redirects a day, fast, without ever giving two different URLs the same code.

## Requirements

Pin these down before drawing anything — the interviewer is watching how you scope.
...
Numbers turn hand-waving into engineering. Assume:
...
A redirect is just a key-value lookup: short code → long URL. You don't need joins, transactions, or a relational schema for the hot path — you need a fast dictionary. The only genuinely hard part is **minting unique codes at 5,000/sec across many machines** without coordination on every request.
...
Use base62 (`a–z`, `A–Z`, `0–9`). Seven characters give 62⁷ ≈ **3.5 trillion** combinations — comfortably past our 365 B ceiling. Six characters (56 B) is close but risky, so budget seven.
...
Write `code → long_url` into a partitioned key-value store (see [sharding](/docs/system-design/part-2-databases/04-sharding)), sharded by the code itself. The code is a great shard key — high cardinality, uniform, and it's exactly what every read queries by.
...
```
                    ┌──────────────┐
   create ─────────▶│  App server  │◀──── ID block from
                    │  (base62)    │      Key-Gen service
                    └──────┬───────┘
                           │ write code→url
                           ▼
                    ┌──────────────┐
                    │  KV store    │  sharded by code
                    │  (365B rows) │
                    └──────────────┘
                           ▲
   click ───▶ [ CDN/edge ] │ miss
...
<Cards>
  ...
  <Card title="Design a News Feed" href="/docs/system-design/part-5-case-studies/02-design-a-news-feed" description="The next case study — fan-out at scale." />
</Cards>
