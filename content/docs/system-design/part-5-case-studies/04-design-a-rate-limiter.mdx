...
description: "Enforcing \"100 requests per minute\" is easy on one server and genuinely hard across a hundred — this is a favorite for a reason."
extractedReferences:
...
A single abusive client starts hammering your API 10,000 times a second, and your database — sized for normal traffic — falls over, taking every other customer down with it. You want a bouncer at the door: "this key gets 100 requests per minute, no more." On one server that's a counter in memory. Done in five minutes.

Then you remember you run **a hundred** API servers behind a load balancer, and any request can hit any one of them. Each server sees only its own slice of the traffic. A client doing 100 req/min spread across 100 servers looks like 1 req/min to each — so your "100/min" limit silently becomes "10,000/min". The whole difficulty is that the count must be **shared**.
...
You can't keep the count in each server's memory — it has to live in **one shared, fast store** that all servers read and increment atomically. Redis is the standard choice: an in-memory store with atomic operations (`INCR`, `EXPIRE`) and sub-millisecond latency. The real design work is then choosing an *algorithm* that trades accuracy against cost, because a naive shared counter has a nasty burst bug.
...
```
        request (key = api_key)
              │
              ▼
   ┌────────────────────┐    atomic INCR / token check
   │  API server (1 of  │◀──────────────┐
   │  100) — thin check │               │
   └─────────┬──────────┘        ┌──────────────┐
             │ under limit       │  Redis       │  key → {tokens, ts}
             ▼                   │  (shared)    │
        pass to app             └──────────────┘
             │ over limit
             └──▶ 429 + Retry-After
...
If the rate-limit store is unreachable, do you allow all traffic (fail open) or block it (fail closed)? Fail-open keeps your product working during a limiter outage but removes your protection exactly when you might need it; fail-closed protects the backend but turns a limiter blip into a full outage. Most public APIs fail open — availability wins — but say it out loud and justify it.
</Callout>
...
- **Coordinated backpressure:** a rate limiter is one layer; pair it with [backpressure and circuit breakers](/docs/system-design/part-4-reliability/04-backpressure-and-circuit-breakers) so the system sheds load gracefully rather than just returning 429s.
