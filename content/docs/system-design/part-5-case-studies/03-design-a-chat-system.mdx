...
description: "Adda adds real-time messaging: messages that arrive instantly, in order, even when a phone drops off the network. Deceptively deep — and it stretches everything the team learned."
extractedReferences:
...
HTTP request/response is pull-shaped — the client asks, the server answers (the same [HTTP model](/docs/system-design/part-1-networking/03-http-and-apis) from Part 1). Chat needs the opposite: a **persistent connection** the server can push down at any moment. That's a **WebSocket**. Once you accept that every online user holds an open socket to some server, Nabila observes, the whole design reorganizes around one question: *when A sends to B, how does A's server find B's socket?*
...
When A sends a message, A's gateway needs to reach B's gateway. A **presence/session service** maps `user → gateway`. A's gateway looks up B, then hands the message to B's gateway — often via an internal [pub/sub](/docs/system-design/part-3-architecture/04-pub-sub) bus, the fan-out pattern from Part 3, so gateways stay decoupled.
...
```mermaid
flowchart LR
  A(["A"]) -->|socket| GW1["Gateway-1"]
  GW1 -->|"lookup B → Gateway-2"| Presence[("presence<br/>user → gateway")]
  GW1 --> Persist[("message store")]
  GW1 --> PubSub["pub/sub"]
  PubSub --> GW2["Gateway-2<br/>or queue if B is offline"]
  GW2 -->|socket| B(["B"])
```
...
- **Message fan-out in groups:** a 1,000-member group turns one send into 1,000 deliveries. This is the same **fan-out** problem as [Adda's feed](/docs/system-design/part-5-case-studies/02-design-a-news-feed) — deliver to online members via their gateways, queue for offline ones.
