...
```mermaid
flowchart TB
  Post(["POST"]) --> Store[("posts store")]
  Post --> Queue["queue"]
  Queue --> Worker["fan-out worker<br/>skip if celebrity"]
  Worker --> FeedCache[("feed cache<br/>user → postId, postId, …")]

  Get(["GET feed"]) --> FeedCache
  FeedCache --> Merge["merge + rank + paginate"]
  Celeb["pull recent celebrity posts"] --> Merge
  Merge --> Client(["client"])
```
