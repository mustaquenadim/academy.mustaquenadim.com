...
```mermaid
flowchart TB
  Upload(["upload — chunks, pre-signed"]) --> Raw[("object storage")]
  Raw -->|"on complete"| Queue["encode queue"]
  Queue --> W1["worker"]
  Queue --> W2["worker"]
  Queue --> W3["worker"]
  W1 --> Out[("object storage<br/>segments + manifest")]
  W2 --> Out
  W3 --> Out
  Out -->|"push hot content"| CDN["CDN"]
  CDN --> Edge["nearest CDN edge"]
  Edge --> Viewer(["viewer"])
  Viewer -->|"measures bandwidth, asks for the next segment at the best quality"| Edge
```
