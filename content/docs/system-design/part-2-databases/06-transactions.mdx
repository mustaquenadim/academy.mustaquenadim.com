---
title: "Transactions"
description: "On Adda a 'like' must record the like AND fire the notification — a crash halfway can't leave one without the other. Transactions make \"all or nothing\" real."
extractedReferences:
...
<Accordion title="An Adda analytics report runs the same SELECT twice inside one transaction and gets different totals because another transaction committed in between. Name the anomaly and the level that fixes it.">
...
<Accordion title="Why does Adda avoid two-phase commit for its cross-shard like+notify even though it gives true atomicity across services?">
