...
description: "When the network splits your servers in two, you must choose: stay consistent or stay available. You can’t have both — that’s CAP."
extractedReferences:
...
You run your database across two datacenters so that if one burns down, the other keeps serving. One afternoon the network link between them goes dark. Both sides are alive and taking traffic — but they can no longer talk to each other. A customer in region A updates their address; a request in region B reads the old one.

Now you face a decision with no comfortable answer. Do you let both sides keep serving, knowing they may disagree? Or do you refuse writes on one side to keep everyone in agreement, making that side effectively down? The network split forced a choice you cannot avoid.

## A first attempt

The instinct is "just keep both up and reconcile later." Sometimes you can — but reconciliation means two versions of the truth, and for a bank balance or a seat on a flight "later" is too late; you might sell the same seat twice.

The opposite instinct — "block writes until the link heals" — keeps everyone consistent but means an outage every time the network hiccups, which on a large system is constantly. Neither instinct is free. What you are bumping into is not a bug you can engineer away; it is a theorem about what is possible when machines cannot communicate.
...
```
   Healthy:            A ───── link ───── B     both consistent + available

   Partition:          A ──╳ (no link) ╳── B
                       │                  │
   CP choice:   accept writes │  reject writes  ← B is unavailable, no stale data
   AP choice:   accept writes │  accept writes  ← both up, may disagree, merge later
```
...
That is why the choice is often made *per operation*, not per system. A shopping cart or a like-count is happy being AP — a stale count for 200 ms harms no one. A payment or an inventory decrement wants CP — better a brief error than double-charging a card. Quorum systems tune this directly with `R + W > N` (read and write replica counts): raise them for consistency, lower them for availability and speed.
...
<Accordion title="Is a single-node database subject to the CAP trade-off?">

Not really — CAP is about *distributed* systems, where nodes can be partitioned from each other. A single node has nothing to partition from, so it can offer both consistency and availability until it simply fails (which is an availability loss, not a CAP dilemma). The trade-off appears the moment you add replicas or shards that must communicate over a network.

</Accordion>

<Accordion title="You are designing a 'like' counter and a bank ledger. Which leans AP and which leans CP, and why?">

The like counter leans AP: availability matters more than exactness, and a count that is briefly off by a few during a partition harms nobody, so you keep serving and converge later. The bank ledger leans CP: reading or writing a stale balance can double-spend money, so during a partition you would rather reject the operation than risk an incorrect answer. Same theorem, opposite costs of being wrong.

</Accordion>

<Accordion title="Someone says their system is 'CA — consistent and available'. Why is that claim suspicious?">
