---
title: "Indexing"
description: "Adda's login page starts timing out, and Shuvo finds the database reading every row to find one. Finding a needle in millions by checking each straw is hopeless — an index is the book's table of contents for your data."
extractedReferences:
...
```mermaid
flowchart LR
  subgraph idx["B-tree index on email — O(log n) to the leaf"]
    direction TB
    Root["m"] --> F["f"]
    Root --> T["t"]
    F --> Leaf["…ana…"]
  ...
  Query(["WHERE email = 'ana@x.com'"]) --> Root
  Leaf -->|"pointer — one extra read"| Heap["Table (heap) · row 8123301<br/>id · name · email …<br/>ana@x.com ✓"]
```
...
<Accordion title="Adda has an index on (last_name, first_name). Which of these queries can use it: filter by last_name, by first_name, by both?">
...
<Accordion title="A query counts 'SELECT count(*) FROM posts WHERE status = published'. When would a covering index help most?">
...
<Accordion title="Why can a hash index beat a B-tree for exact lookups but be useless for Adda's 'posts from the last 7 days' feed?">
