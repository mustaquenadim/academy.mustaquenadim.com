...
```mermaid
flowchart LR
  subgraph rel["Relational (normalized) — 1 write updates a name once"]
    direction LR
    Customers["customers"] -->|join| Page["order page"]
    Orders["orders"] -->|join| Page
    LineItems["line_items"] -->|join| Page
  end

  subgraph doc["Document (denormalized) — 1 name lives in every order"]
    direction LR
    Read(["one read"]) --> Doc["orders/4711<br/>id: 4711<br/>customer: { name }<br/>items: [ … ]"]
  end
```
