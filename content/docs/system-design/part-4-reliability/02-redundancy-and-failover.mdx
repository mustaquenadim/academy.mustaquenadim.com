...
description: "Everything Adda runs on fails eventually — the question is whether Fahim notices. Mou's answer: a spare that's always ready to take over."
extractedReferences:
...
```mermaid
flowchart TB
  subgraph ap["ACTIVE-PASSIVE — on failure: promote standby (seconds of blip)"]
    direction TB
    C1(["clients"]) --> LB1["LB"]
    LB1 -->|ok| P["PRIMARY<br/>serves all"]
    LB1 -.->|"dead — idle, warm"| S["STANDBY"]
  end

  subgraph aa["ACTIVE-ACTIVE — on failure: the survivor absorbs the load"]
    direction TB
    C2(["clients"]) --> LB2["LB"]
    LB2 --> N1["N1<br/>serves ~50%"]
    LB2 --> N2["N2<br/>serves ~50%"]
  end
```
...
<Accordion title="Adda's standby has been idle for months. Why might failover still fail when Mou needs it?">

Idle standbys rot. The config may have drifted from the primary, its software version may be stale, its data may have fallen behind in replication, or its disk may have quietly filled. Because nothing exercises it, none of this surfaces until the moment you promote it — and then it's a second outage on top of the first. This is why active-active (where every node is constantly proven in production) or regular chaos drills beat a cold standby nobody's watched take real traffic.

</Accordion>

<Accordion title="Active-active doubles Adda's capacity, so why not run every node at 90% utilization?">

Because when one node dies, its traffic lands on the survivors. Two nodes at 90% each mean the survivor would need 180% — it collapses, and now Adda has a cascading outage instead of a clean failover. You must leave enough headroom that the remaining nodes can absorb the dead one's share: for 2 nodes, cap each at ~50%; for N nodes surviving one loss, cap at roughly (N−1)/N. That spare headroom is the real cost of redundancy.

</Accordion>

<Accordion title="Why is failing over Adda's database far harder than failing over a web server?">
