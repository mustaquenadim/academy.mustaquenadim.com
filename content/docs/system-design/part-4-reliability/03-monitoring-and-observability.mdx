...
description: "Adda got slow for hours and nothing paged — because nobody was watching the right thing. Mou can't fix what she can't see, so she gives Adda eyes: logs, metrics, and traces."
extractedReferences:
...
```mermaid
flowchart TB
  subgraph path["request path — trace_id=abc"]
    direction LR
    Req(["request"]) --> GW["API GW<br/>span"] --> Orders["Orders<br/>span"] --> Payments["Payments<br/>span"] --> DB[("DB")]
  end

  path --> Metrics["METRICS: p99 latency spikes at 09:14<br/>'something is wrong, now'"]
  Metrics -->|"pivot by time"| Traces["TRACES: abc → GW 5ms · Orders 8ms · Payments 900ms<br/>Payments is the slow hop"]
  Traces -->|"pivot by trace_id"| Logs["LOGS: trace_id=abc, svc=payments,<br/>msg='upstream timeout' — the why"]
```
