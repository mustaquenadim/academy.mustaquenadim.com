...
```mermaid
flowchart TB
  subgraph hops["packets hop router → router → router — each hop only knows the next hop"]
    direction LR
    Carrier["carrier"] --> IXP["IXP"] --> Backbone["backbone"] --> ISP["ISP"]
  end

  Phone(["Fahim's phone"]) -->|"1. DNS: adda.com → IP"| Resolver["Resolver"]
  Phone -->|"2. TCP + TLS handshake<br/>3. HTTP GET /"| Carrier
  ISP --> Adda["Ria's laptop<br/>(Adda)"]
```
