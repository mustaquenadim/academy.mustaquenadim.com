...
flowchart LR
  subgraph without["Without CDN"]
    direction LR
    User1(["Sydney user"]) -->|"~200 ms round trip<br/>every request, every user"| Origin1[("Dhaka origin")]
  end

  subgraph withCdn["With CDN"]
    direction LR
    User2(["Sydney user"]) -->|"~5 ms"| Edge["Sydney edge"]
    Edge -->|"cache miss, once"| Origin2[("Dhaka origin")]
    User3(["next Sydney users"]) -->|"cache hit, no origin"| Edge
  end
```
