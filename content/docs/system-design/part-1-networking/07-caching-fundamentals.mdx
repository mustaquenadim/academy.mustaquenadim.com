...
description: "Adda's database is melting under a flood of identical reads for the same hot posts. Ria and Tanvir stop re-answering the same question — they answer it once and remember the answer."
extractedReferences:
...
<Accordion title="Why does even a 95% cache hit ratio leave Adda's latency dominated by misses?">
...
The old value is still in the cache (cache-aside) and the TTL has not expired, so reads keep returning the stale copy. Fixes: **invalidate** the entry on write (delete it, so the next read repopulates), switch that data to **write-through** (update cache and DB together so it is never stale), or shorten the TTL to bound how long staleness can last. Which Ria chooses depends on how much staleness this data can tolerate — a profile name, very little.

</Accordion>

<Accordion title="What is a cache stampede, and how does Adda prevent it?">

When a hot key like the trending list expires, many concurrent requests all miss at once and hammer the database with the same expensive query simultaneously — the very flood the cache existed to prevent. Prevent it by letting only one request recompute while others wait (a lock / "single-flight"), refreshing popular keys *before* they expire, or adding small random jitter to TTLs so keys do not all expire together. The danger is many misses landing at the same instant.
