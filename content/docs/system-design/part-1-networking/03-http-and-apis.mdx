---
title: "HTTP & APIs"
description: "A friend builds a tiny mobile client for Adda, and suddenly two very different programs need to agree on how to talk. HTTP is that shared language; an API is the menu of what you can ask for."
extractedReferences:
...
<Accordion title="Why does statelessness make horizontal scaling easy for Adda?">

Because no server holds context that only *it* knows, any request can be routed to any server behind a load balancer. Ria can add or remove machines freely, and one server crashing does not lose a user's session. The catch is that shared state (sessions, drafts) must move to a shared store like Redis or a database — you have relocated the state, not eliminated it.

</Accordion>

<Accordion title="A post request from the mobile app times out. Is it safe to retry?">

Only if it is idempotent. A raw `POST /posts` is not — the first attempt may have succeeded before the timeout, so a blind retry can double-post. Make it safe by sending an **idempotency key**: Adda records the key, and a retry with the same key returns the original result instead of creating a second post. Idempotent methods (`GET`, `PUT`, `DELETE`) are retry-safe by definition.

</Accordion>

<Accordion title="The client gets a 503, then a 500, then a 404. How should it react to each?">
